...
export default function AmplifierConnectionClient({
  initialConnected,
}: {
  ...
  const handleTest = async () => {
    ...
    setTestResult(null);
    try {
      const res = await fetch("/api/connections/test/amplifier");
      const data = await res.json();
      if (res.ok && data.connected) {
        setTestResult({
          ok: true,
          message: `Connected — ${data.items} inventory items found`,
        });
      } else {
        setTestResult({
          ok: false,
  ...
  const handleSync = async () => {
    ...
    setSyncResult(null);
    try {
      const res = await fetch("/api/sync/3pl-inventory", { method: "POST" });
      const data = await res.json();
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                3PL fulfillment and inventory management
              </p>
            </div>
          </div>
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3">
          ...
          >
            ...
            />
            {syncing ? "Syncing..." : "Sync 3PL Inventory"}
          </button>
        </div>
