...
export default function ThreePLReviewClient() {
  ...
  useEffect(() => {
    const includeDismissed = filter === "dismissed" || filter === "all";
    fetch(`/api/external-skus?source=3pl${includeDismissed ? "&include_dismissed=true" : ""}`)
      .then((r) => r.json())
      .then((data) => {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Review all Amplifier SKUs. Map products or dismiss retired items.
        </p>
      </div>
