...
export default function AmazonReviewClient() {
  ...
  const setStatus = async (ids: string[], status: string) => {
    ...
    setSkus((prev) => prev.map((s) => (ids.includes(s.id) ? { ...s, status } : s)));
    setSaving((prev) => { const next = new Set(prev); ids.forEach((id) => next.delete(id)); return next; });
  };

  const dismissAllZero = () => {
    const zeroIds = filtered.filter((s) => (s.quantity ?? 0) === 0 && !s.mapped && s.status === "active").map((s) => s.id);
    if (zeroIds.length > 0) setStatus(zeroIds, "dismissed");
  };
  ...
  if (loading) return <div className="flex items-center justify-center py-20 text-gray-400">Loading Amazon listings...</div>;
    ...
    <div className="space-y-4">
        ...
        <div className="flex gap-1 flex-wrap">
          {filters.map((f) => (
            <button key={f.key} onClick={() => setFilter(f.key)}
              className={`rounded-lg px-3 py-2 text-xs font-medium transition-colors ${filter === f.key ? "bg-brand-600 text-white" : "bg-white border border-gray-300 text-gray-600 hover:bg-gray-50"}`}
            >{f.label}</button>
          ))}
        </div>
        <div className="relative flex-1 max-w-sm">
          <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
          <input type="text" placeholder="Search by title, ASIN, or SKU..." value={search} onChange={(e) => setSearch(e.target.value)}
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.length === 0 ? (
                ...
                <tr key={sku.id} className={`hover:bg-gray-50 ${sku.status !== "active" && !sku.mapped ? "opacity-50" : ""}`}>
                  ...
                  <td className="px-4 py-2.5 max-w-xs"><p className="text-sm font-medium text-gray-900 truncate">{sku.title || "—"}</p></td>
                  ...
                  <td className="px-4 py-2.5 text-right">
                    <div className="flex items-center justify-end gap-1">
                      {sku.status === "dismissed" || sku.status === "discontinued" ? (
                        <button onClick={() => setStatus([sku.id], "active")} disabled={saving.has(sku.id)}
                        ...
                        <>
                          ...
                          <button onClick={() => setStatus([sku.id], "dismissed")} disabled={saving.has(sku.id)}
      ...
      {mappingSku && (
        <QuickMapModal
          ...
          label={mappingSku.seller_sku}
          title={mappingSku.title}
          onMapped={(name) => handleMapped(mappingSku, name)}
