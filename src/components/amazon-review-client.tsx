...
export default function AmazonReviewClient() {
  ...
  if (loading) {
    return <div className="flex items-center justify-center py-20 text-gray-400">Loading Amazon listings...</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Review all Amazon ASINs. Map products or dismiss retired listings.</p>
      </div>
                  ...
                  <tr key={sku.id} className={`hover:bg-gray-50 ${sku.status !== "active" && !sku.mapped ? "opacity-50" : ""}`}>
                    ...
                    <td className="px-4 py-2.5 text-right">
                      {sku.status === "dismissed" || sku.status === "discontinued" ? (
                        <button
                          onClick={() => setStatus([sku.id], "active")}
                        ...
                        <button
                          onClick={() => setStatus([sku.id], "dismissed")}
                          disabled={saving.has(sku.id)}
